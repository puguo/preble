...
class Simulation:
    ...
    def get_waiting_queue(self):
        return {i: len(rt.queue) for i, rt in enumerate(self.runtimes)}
...
class SendRequestEvent:
    ...
    def process_event(self, simulator: Simulation):
        ...
        waiting_queues = simulator.get_waiting_queue()
        gpu_selected = scheduler.runtime_selector(
            text=self.request["text"],
            request_id=self.request["rid"],
